//
// Created by nitzan on 26/12/2022.
//
...
int pm_addstring(pm_t * pm_to_add_to,unsigned char* string_to_be_added, _size_t n){///adds strings , if the value of the char i in the string already exists we don't make another one.
        ...
        for (dbllist_node_t* j = dbllist_head(new_state->_transitions ) ; j != NULL; j = dbllist_next(j)){
            ...
            if(dbllist_data(j)!=NULL){
                ...
                if(pm_goto_get(fail_state,((pm_labeled_edge_t*)dbllist_data(j))->label)!=NULL){
                    ...
                    if(dbllist_head(child_state->fail->output)!=NULL){
                        if(dbllist_data(dbllist_head(child_state->fail->output))!=NULL){
                            if(dbllist_head(child_state->output)==NULL){
                                dbllist_head(child_state->output) = dbllist_head(child_state->fail->output);
                                flag_output_head_null = 1;
                            }
                            else if(dbllist_head(child_state->output)==dbllist_tail(child_state->output)){
                                dbllist_next(dbllist_head(child_state->output)) = dbllist_head(
                                        child_state->fail->output);
                            }
                            if(!flag_output_head_null){
...
dbllist_t* pm_fsm_search(pm_state_t *state,unsigned char *where_to_search,_size_t length){
    ...
    if(state == NULL ||where_to_search == "\0")return returned_pm_matches;
...
//    cur_state = state;
    for (int i = 0; i < length; i++) {
        if(pm_goto_get(cur_state,where_to_search[i])!=NULL) {
            ...
            if (dbllist_head(cur_state->output) != NULL) {
...
//            free(dbllist_head(cur_state->output));
            ...
            if(cur_state->output!=NULL&&dbllist_head(cur_state->output)!=NULL){
                free(dbllist_head(cur_state->output));
                dbllist_head(cur_state->output) = NULL;
                dbllist_tail(cur_state->output) = NULL;
