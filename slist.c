...
//            return;
//        }
        for (dbllist_node_t *head = dbllist_next(dbllist_head(List));
            dbllist_data(head) != dbllist_data(dbllist_tail(List)); head = dbllist_next(head)) {
            free(dbllist_data(dbllist_prev(head)));
            free(dbllist_prev(head));
        }
        free(dbllist_data(dbllist_prev(dbllist_tail(List))));
        free(dbllist_prev(dbllist_tail(List)));
        free(dbllist_data(dbllist_tail(List)));
...
int dbllist_remove(dbllist_t* List, dbllist_node_t* node_to_remove ,dbllist_destroy_t dealloc){
    ...
    if(dbllist_head(List)==NULL||node_to_remove==NULL)return -1;
        ...
        if(j==node_to_remove){
            if(j == dbllist_head(List)){
                if(j==dbllist_tail(List)){
                    ...
                    if(dealloc ==DBLLIST_FREE_DATA&&dbllist_head(List)!=NULL){
                        if(dbllist_prev(dbllist_head(List))!=NULL) {
